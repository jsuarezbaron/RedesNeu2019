...
class PerceptronMulticapa(object):
    ...
    def backward(self, mu, output, target, eta):
        ...
        for i in range(self.neu_):
        	for j in range(self.in_):
        		self.delta1_[i] = (2 * self.v_[i] *(1 - self.v_[i])) * np.dot(self.delta2_,self.w2_[,i]) ### 2*v*(1-v) es la derivada de la sigmoidea
    ...
    def net_input(self, X, w):
        return np.dot(X, w[1:]) + w[0]		#w[0] es el bias
